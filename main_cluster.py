...
def eval_genomes(genomes, config):
    ...
    for idx,i in enumerate(nodeList):
        i.soc.send("ARE_DISCONNECT")
        are = bool(i.soc.recive(int))
        if are:
            print("Disconnecting node ", i.name)
            for jdx,j in enumerate(server.clientList):
                if j == i.soc:
                    server.clientList.pop(jdx)
            nodeList.pop(idx)
            if len(nodeList) == 0:
                print("Paused waiting for new client")
            is_NodeChange = True
    first = True
    while len(nodeList) == 0 or first:
        ...
        if is_NodeChange and len(nodeList) != 0:# re calculate genome distrubution
